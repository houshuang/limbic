#!/usr/bin/env python3
"""Evaluation harness for autoresearch creative loop on knowledge_map propagation.

Outputs a single scalar: mean heuristic accuracy across topologies and K values.
Also outputs the gap to pgmpy Bayesian ceiling.

Usage:
    python3 experiments/autoresearch_eval.py
    # Output: SCORE=0.6683 CEILING=0.7209 GAP=0.0526
"""
...
def accuracy(graph, truth, assessed, propagator="heuristic"):
...
for topo_name, make_fn in TOPOLOGIES.items():
    ...
    for K in K_VALUES:
        for trial in range(N_TRIALS):
            ...
            h_total += accuracy(graph, truth, assessed, "heuristic")
            b_total += accuracy(graph, truth, assessed, "bayesian")
            n_evals += 1

h_mean = h_total / n_evals
b_mean = b_total / n_evals
gap = b_mean - h_mean

print(f"SCORE={h_mean:.4f} CEILING={b_mean:.4f} GAP={gap:.4f}")
