...
|---|---|---|
| **`limbic.amygdala`** | **Find patterns** — embedding with whitening, vector + FTS5 hybrid search, RRF fusion, reranking, novelty scoring, clustering, document similarity, knowledge mapping, temporal reasoning, Wikidata lookup, retrieval evaluation, serendipity | [→](limbic/amygdala/README.md) |
| **`limbic.hippocampus`** | **Manage changes** — proposals with a review lifecycle, cascade merges that relink every reference, veto-gate dedup, composable validation, file-locked YAML store, deterministic Wikidata entity resolution | [→](limbic/hippocampus/README.md) |
| **`limbic.cerebellum`** | **Verify correctness** — resumable LLM batch processing, tiered orchestration with escalation, audit logs, cross-project cost tracking, Claude/Codex CLI wrappers, agent isolation, windowed extraction | [→](limbic/cerebellum/README.md) |
| **`limbic.drive`** | **Decide what to do first** — gate a plan against a calibration-first policy before it spends anything | [→](limbic/drive/README.md) |
...
external services — and the defaults were measured rather than guessed.
...
```bash
pip install git+https://github.com/houshuang/limbic.git          # core
pip install "limbic[hippocampus] @ git+..."                      # + YAML store
pip install "limbic[llm] @ git+..."                              # + Gemini/Anthropic/OpenAI
pip install "limbic[temporal] @ git+..."                         # + full EDTF dates
```
...
- **cerebellum** wraps the expensive, failure-prone parts: `batch` and
  `orchestrator` for resumability and budget, `cost_log` for attribution,
  `claude_cli`/`codex_cli`/`sandbox` for driving an agent CLI safely.
- **drive** is pure policy — no I/O, no model calls.

Design principles: no external services; opt-in complexity (core needs only
numpy + sentence-transformers); storage-agnostic callbacks; numpy arrays
everywhere; two-tier caching (in-memory LRU + optional SQLite).
